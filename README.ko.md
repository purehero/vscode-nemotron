...
- **파일 도구(에이전트)** — AI가 워크스페이스 파일을 직접 다룹니다:
  ...
  - `run_command` — 터미널 명령 실행 후 출력 확인 (디버깅; 승인 필요). 예: "실행해보고 에러 고쳐줘". 지속 셸에서 포그라운드로 실행되어 cd/venv 가 유지되며, 타임아웃(기본 60초)을 넘기면 실패시키지 않고 백그라운드로 다시 띄워 잡 id 를 반환합니다. `background: true` 면 기다리지 않고 즉시 분리(개발 서버 등)
  - `check_command` — 백그라운드 명령의 새 출력·진행/종료 상태 조회 (직전 조회 이후 출력만 반환)
  - `stop_command` — 백그라운드 명령 중단
