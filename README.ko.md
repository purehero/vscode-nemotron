...
- **파일 도구(에이전트)** — AI가 워크스페이스 파일을 직접 다룹니다:
  ...
  - `run_command` — 터미널 명령 실행 후 출력 확인 (디버깅; 승인 필요). 예: "실행해보고 에러 고쳐줘". 모든 명령이 백그라운드 잡으로 실행되어, 빠른 명령은 결과를 바로 돌려주고 빌드·설치·테스트처럼 오래 걸리는 명령은 타임아웃으로 실패하지 않고 잡 id 를 반환합니다. `background: true` 면 기다리지 않고 즉시 분리(개발 서버 등)
  - `check_command` — 실행 중인 명령의 새 출력·진행/종료 상태 조회 (직전 조회 이후 출력만 반환)
  - `stop_command` — 실행 중인 명령 중단
